...
class YelpSpider(Spider):
    ...
    start_urls = [
        "http://www.yelp.com/search?find_desc=&find_loc=Urbana-Champaign%2C+IL&start=1"]
    for i in range(1, 100):
        start_urls.append(
            "http://www.yelp.com/search?find_desc=&find_loc=Urbana-Champaign%2C+IL&start="+str(i*10))

    def parse(self, response):
        sel = Selector(response)
        sites = sel.xpath(
            '//*[@id="super-container"]/div[3]/div[3]/div[1]/div/div[1]/ul/li')
        items = []
        ...
        for site in sites:
            ...
            item['category'] = site.xpath(
                'div/div[1]/div[2]/div[2]/span[2]/a/text()').extract()

            rating = site.xpath(
                'div/div[1]/div[2]/div[1]/div/i/@title').extract()
            if len(rating) > 0:
                item['rating'] = rating[0].split(' ')[0]
            else:
                item['rating'] = ''

            price_rating = site.xpath(
                'div/div[1]/div[2]/div[2]/span[1]/span/text()').extract()
            if len(price_rating) > 0:
                item['price_rating'] = unicodedata(str(len(price_rating[0])))
            else:
                item['price_rating'] = ''
            ...
            fav_comment_user = site.xpath(
                'div/div[3]/div[1]/div/a/img/@alt').extract()
            if len(fav_comment_user) > 0:
                item['fav_comment_user'] = fav_comment_user[0]
            else:
                item['fav_comment_user'] = ''

            fav_comment_content = site.xpath(
                'div/div[3]/div[2]/p/text()').extract()
            if len(fav_comment_content) > 0:
                item['fav_comment_content'] = fav_comment_content[0]
            else:
                item['fav_comment_content'] = ''

            fav_comment_user_face = site.xpath(
                'div/div[3]/div[1]/div/a/img/@src').extract()
            if len(fav_comment_user_face) > 0:
                item['fav_comment_user_face'] = u'http:' + \
                    fav_comment_user_face[0]
            else:
